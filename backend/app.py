...
def upload_data():
    ...
    try:
        logger.info("接收到文件: %s", file.filename)
        data = load_data(file)
        ...
        logger.info("数据处理成功")
        logger.info("处理后的数据: %s", processed_data)
...
def visualize_data():
    try:
        ...
        logger.info("接收到可视化请求: chart_type=%s, column=%s", chart_type, column)
        ...
        if chart_type == "histogram":
            ...
            fig = create_scatter_plot(
                df, x_column, y_column, f"{x_column}与{y_column}的散点图"
            )
        elif chart_type == "line":
            x_column = request.json["xColumn"]
            y_column = request.json["yColumn"]
            fig = create_line_plot(
                df, x_column, y_column, f"{x_column}与{y_column}的折线图"
            )
        elif chart_type == "pie":
            ...
            logger.warning("无效的图表类型: %s", chart_type)
...
def perform_regression_analysis():
    try:
        ...
        logger.info(
            "接收到回归分析请求: x_columns=%s, y_column=%s", x_columns, y_column
        )
        ...
        if not all(col in df.columns for col in x_columns + [y_column]):
            logger.warning("列名不存在")
            return jsonify({"error": "指定的列名不存在"}), 400

        if not all(
            pd.api.types.is_numeric_dtype(df[col]) for col in x_columns + [y_column]
        ):
            logger.warning("非数值类型列")
...
def time_series_analysis_route():
    try:
        ...
        logger.info("接收到时间序列分析请求: column=%s, periods=%s", column, periods)
        ...
        if column not in df.columns:
            logger.warning("列名不存在: column=%s", column)
            return jsonify({"error": "指定的列名不存在"}), 400
...
def group_and_aggregate_data():
    try:
        ...
        logger.info(
            "接收到数据分组聚合请求: group_by=%s, agg_column=%s, agg_func=%s",
            group_by,
            agg_column,
            agg_func,
        )
...
def preprocess_data_route():
    try:
        ...
        logger.info("接收到数据预处理请求: method=%s, columns=%s", method, columns)
